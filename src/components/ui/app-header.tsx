...
type NavKey = "home" | "products" | "calculator";

interface Props {
  /**
   * Literal title, used when no `titleKey` is given. Pages still in English pass
   * this and are simply unaffected by the language toggle.
   */
  title: string;
  backHref?: string;
  backLabel?: string;
  /**
   * Dictionary keys under `nav`. Preferred over the literals above, which cannot
   * follow the toggle because they are set in a server component.
   */
  titleKey?: NavKey;
  backLabelKey?: NavKey;
}
    ...
    <header className="sticky top-0 z-40 w-full backdrop-blur-xl bg-white/70 dark:bg-zinc-950/70 border-b border-slate-200/50 dark:border-white/10 transition-all duration-300">
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-6 lg:px-8">
        ...
        >
          ...
          <span className="hidden sm:inline-block">{backLabelKey ? t(backLabelKey) : backLabel}</span>
        </Link>
        <div className="h-4 w-px bg-slate-200 dark:bg-zinc-800 mx-2"></div>
        <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white flex-1">
